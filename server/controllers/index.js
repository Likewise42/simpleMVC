...
const searchNameDog = (req, res) => {
  ...
  return Dog.findByName(req.query.name, (err, doc) => {
    ...
    const dDoc = doc;
    dDoc.age++;
    const savePromise = dDoc.save();

    // send back the name as a success for now
    savePromise.then(() => res.json({ name: dDoc.name, breed: dDoc.breed, age: dDoc.age }));

    // if save error, just return an error for now
    savePromise.catch(err2 => res.json({ err2 }));

    return res.json({ name: dDoc.name, breed: dDoc.breed, age: dDoc.age });
  });
};
...
const notFound = (req, res) => {
  ...
  // Additionally, you don't need .jade because you registered the file type
  // in the app.js as jade. Calling res.render('index')
  // actuaxlly calls index.jade. A second parameter of JSON can be passed into
